...
public class JSONUtil {
	private static ObjectMapper objectMapper = new ObjectMapper(new JsonFactory().configure(Feature.ALLOW_UNQUOTED_CONTROL_CHARS, true));
	...
	public static List toList(String json) throws JSONException{
		json = json.replaceAll("(?i)true", "true");
		json = json.replaceAll("(?i)false", "false");
		json = json.replaceAll(",\\s*,",",null,");
		json = json.replaceAll(",\\s*,",",null,");
		json = json.replaceAll(",\\s*]",",null]");
		json = json.replaceAll("\\[\\s*,","[null,");
		if(json == null
